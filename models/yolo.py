...
from models.sparseinst import BaseIAMDecoder
...
class RGPSegment(nn.Module):
    ...
    def __init__(self, c1, c2):
        super(RGPSegment, self).__init__()
        self.cv1 = Conv(c1, c2)
        # self.detect = ISDetect.forward
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head'] + d['segHead']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, ConvTranspose, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
        ...
        if i == 0:
            ch = []
        ch.append(c2)
    return nn.Sequential(*layers), sorted(save), sorted(d['output_idx'])
